  ...
  <div>
    <v-container>
        ...
        <v-col cols="12">
          <v-card>
            ...
            <v-card-text>
              <v-data-table
                :headers="headers"
                :items="categoryStore.incomeCategories"
                :search="search"
                :loading="categoryStore.loading"
                loading-text="Loading categories..."
                no-data-text="No income categories found"
                class="elevation-1"
              >
      ...
      <v-row>
        <v-col cols="12">
          <v-card>
            <v-card-title>Add New Income Category</v-card-title>
            <v-card-text>
...
const deleteCategory = async (category) => {
  if (confirm(`Are you sure you want to delete category "${category.name}"?`)) {
    try {
      await categoryStore.deleteCategory(category.id, "income");
